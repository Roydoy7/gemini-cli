...
/**
 * Wrapper for LmStudioClient with auto-save and timeout management
 */
class ClientWrapper {
  ...
  resetIdleTimer(): void {
    ...
    this.idleTimer = setTimeout(() => {
      console.log(
        `[LmStudioClientWrapper] Client for session ${this.sessionId} idle for 15 minutes, releasing...`,
      );
...
/**
 * LmStudioClientPool - Manages multiple LmStudioClient instances for concurrent sessions
 *
 * Features:
 * - Each session has its own LmStudioClient instance
 ...
 */
export class LmStudioClientPool implements IClientPool {
  ...
  /**
   * Get or create an LmStudioClient for the specified session
   */
  async getOrCreate(sessionId: string): Promise<IClient> {
    ...
    // Create new client
    console.log(
      `[LmStudioClientPool] Creating new client for session: ${sessionId}`,
    );
    const client = new LmStudioClient(this.config);
    await client.initialize();
    await client.updateToolsForCurrentRole();

    // Restore session history from SessionManager
    console.log(
      `[LmStudioClientPool] Restoring session history for session: ${sessionId}`,
    );
    ...
    console.log(
      `[LmStudioClientPool] Client created for session ${sessionId}. Pool size: ${this.clients.size}`,
    );
  ...
  release(sessionId: string): void {
    ...
    if (wrapper) {
      console.log(
        `[LmStudioClientPool] Releasing client for session: ${sessionId}`,
      );

      // Save before releasing
      wrapper.save().catch((error) => {
        console.error(
          `[LmStudioClientPool] Failed to save session ${sessionId} before release:`,
          error,
        );
      });
      ...
      console.log(
        `[LmStudioClientPool] Pool size after release: ${this.clients.size}`,
      );
    }
  }
  ...
  async clear(): Promise<void> {
    console.log(
      `[LmStudioClientPool] Clearing pool with ${this.clients.size} clients`,
    );
