...
export class MultiModelSystem {
  ...
  private static readonly MODEL_FORMATS: Record<
    string,
    'openai' | 'harmony' | 'gemini' | 'qwen'
  > = {
  ...
  private getModelFormat(): 'openai' | 'harmony' | 'gemini' | 'qwen' {
    ...
    for (const [key, format] of Object.entries(
      MultiModelSystem.MODEL_FORMATS,
    )) {
      if (
        modelName.includes(key.toLowerCase()) ||
        key.toLowerCase().includes(modelName)
      ) {
        console.log(
          `[MultiModelSystem] Model ${modelName} matched pattern ${key}, using ${format} format`,
        );
        return format;
      }
    }
    ...
    console.log(
      `[MultiModelSystem] Unknown model ${modelName}, defaulting to OpenAI format`,
    );
    return 'openai';
  }
...
<|end|>`;
      ...
      return {
        ...
        content: JSON.stringify({
          __gemini_function_response: functionResponseData,
        }),
        tool_call_id: toolCallId,
        name: toolName,
        timestamp: new Date(),
  ...
  constructor(config: Config) {
    ...
    this.currentProvider =
      this.configManager.getDefaultProviderConfig() || null;
  }
  ...
  async initialize(): Promise<void> {
    ...
    if (this.currentProvider) {
      ...
      console.log(
        `[MultiModelSystem] Initialized default provider: ${this.currentProvider.type}-${this.currentProvider.model}`,
      );
    }
  }
  ...
  async sendMessage(
    ...
    while (turnCount < maxTurns) {
      ...
      const enhancedMessages = await this.enhanceMessagesWithRole(
        [...currentHistory],
        roleId,
      );
      ...
      for (const toolCall of toolCalls) {
        ...
        try {
          ...
          toolResults.push(
            `Tool ${toolCall.name} error: ${getErrorMessage(error)}`,
          );
        }
      }
  ...
  async *sendMessageStream(
    ...
    while (true) {
      ...
      if (turnCount > MAX_TURNS) {
        console.error(
          `[MultiModelSystem] Reached absolute maximum turns (${MAX_TURNS}), stopping to prevent infinite loop.`,
        );
      ...
      if (duplicateCount > MAX_DUPLICATE_CALLS) {
        console.error(
          `[MultiModelSystem] Reached maximum duplicate tool calls (${duplicateCount}), stopping to prevent infinite loop.`,
        );
      ...
      const userTurnsCount = fullHistory.filter(
        (m) => m.role === 'user',
      ).length;

      // Check if we need to compress before processing (like Gemini client does)
      const compressionResult =
        userTurnsCount >= MIN_TURNS_BEFORE_COMPRESSION_CHECK
      ...
      if (
        ...
        const compressionInfo: CompressionInfo = {
          ...
          compressionRatio:
            compressionResult.newTokenCount /
      ...
      const responseStream = provider.sendMessageStream(
        enhancedMessages,
        signal,
      );
      ...
      for await (const event of responseStream) {
        ...
        if (event.type === 'content_delta' && event.content) {
          ...
          console.log(
            `[MultiModelSystem] Collected tool call: ${event.toolCall.name}`,
          );
          ...
          console.log(
            `[MultiModelSystem] Provider compression event:`,
            event.compressionInfo,
          );
          yield event;
          ...
          if (event.error?.stack) {
            console.error(
              `[MultiModelSystem] Error stack: ${event.error.stack}`,
            );
          }
          ...
          if (assistantContent.trim()) {
            ...
            console.log(
              `[MultiModelSystem] Saved assistant response before error (${assistantContent.length} chars, discarded ${assistantToolCalls.length} tool calls)`,
            );
          }
          ...
          if (toolCallRequests.length > 0) {
            console.warn(
              `[MultiModelSystem] Discarding ${toolCallRequests.length} tool call request(s) due to error`,
            );
      ...
      if (toolCallRequests.length > 0) {
        ...
        for (const requestInfo of toolCallRequests) {
          ...
          const isDuplicate = toolCallHistory.some(
            (prevCall) =>
              prevCall.name === callSignature.name &&
              JSON.stringify(prevCall.args) ===
                JSON.stringify(callSignature.args) &&
          ...
          if (isDuplicate) {
            duplicateCount++;
            console.warn(
              `[MultiModelSystem] Detected duplicate tool call: ${requestInfo.name} (duplicate count: ${duplicateCount})`,
            );
          } else {
            // Reset duplicate count for different calls
            if (duplicateCount > 0) {
              console.log(
                `[MultiModelSystem] Tool call changed, resetting duplicate count from ${duplicateCount} to 0`,
              );
              duplicateCount = 0;
            }
          }
        ...
        if (this.toolConfirmationHandler) {
          await new Promise<void>((resolve, reject) => {
            const scheduler = new CoreToolScheduler({
              ...
              onToolCallsUpdate: async (toolCallsUpdate) => {
                for (const toolCall of toolCallsUpdate) {
                  ...
                  if (
                    ...
                    console.log(
                      `[MultiModelSystem] Collected tool response for ${toolCall.request.name} (status: ${toolCall.status})`,
                    );
                  }
                }
              },
              ...
              onAllToolCallsComplete: async (completedToolCalls) => {
                ...
                console.log(
                  `[MultiModelSystem] All ${completedToolCalls.length} tool calls completed`,
                );
                resolve();
              },
            });
          ...
          for (const toolResponseMessage of executedToolResponses) {
            ...
            const content =
              typeof toolResponseMessage.content === 'string'
        ...
        console.log(
          `[MultiModelSystem] Tool execution completed for ${toolCallRequests.length} tools. Continuing conversation...`,
        );
        ...
        if (assistantContent.trim()) {
          ...
          SessionManager.getInstance().addHistory(assistantMessage);
          console.log(
            `[MultiModelSystem] Saved assistant response without tool calls (${assistantContent.length} chars)`,
          );
        }
        ...
        if (currentSessionId) {
          sessionManager.autoGenerateTitle(currentSessionId).catch((error) => {
            console.error(
              '[MultiModelSystem] Failed to auto-generate title:',
              error,
            );
          });
        }
        ...
        const skipNextSpeakerCheck = this.config.getSkipNextSpeakerCheck
        ...
        if (
          ...
          try {
            ...
            const nextSpeakerResult = await checkNextSpeaker(
              ...
              async (checkMessages, checkModel) => {
                ...
                const utilityProvider = await this.getOrCreateUtilityProvider(
            ...
            console.log(
              '[MultiModelSystem] Next speaker check result:',
              nextSpeakerResult,
            );

            if (
              nextSpeakerResult &&
              nextSpeakerResult.next_speaker === 'model'
            ) {
              // Model should continue, send "Please continue." message
              console.log(
                '[MultiModelSystem] Model should continue, sending continuation prompt (depth:',
                continuationDepth,
                ')',
              );
            ...
            console.warn(
              '[MultiModelSystem] Next speaker check failed:',
              error,
            );
          ...
          console.warn(
            `[MultiModelSystem] Reached max continuation depth (${MAX_CONTINUATION_DEPTH}), stopping continuation`,
          );
        }
        ...
        if (assistantContent.trim()) {
          ...
          if (currentSessionId) {
            this.generateIntelligentTitle(currentSessionId).catch((error) => {
              console.error(
                '[MultiModelSystem] Failed to generate title:',
                error,
              );
            });
          }
  ...
  private async generateIntelligentTitle(sessionId: string): Promise<void> {
    ...
    try {
      ...
      const displayMessages = sessionManager
...
Title:`;
      ...
      if (
        ...
        console.log(
          `[MultiModelSystem] Generated title for session ${sessionId}: ${generatedTitle}`,
        );
  ...
  async getAvailableModels(
    ...
    for (const config of configs) {
      try {
        ...
        console.error(
          `Failed to fetch models for provider ${config!.type}:`,
          error,
        );
      }
    }
  ...
  setApprovalMode(mode: 'default' | 'autoEdit' | 'yolo'): void {
    ...
    if (this.activeToolScheduler && approvalModeValue !== previousMode) {
      ...
      this.activeToolScheduler
        ...
        .catch((error) => {
          console.error(
            'Error reevaluating pending tools after approval mode change:',
            error,
          );
        });
    }
  ...
  private async getOrCreateProvider(
    ...
    console.log(
      `[MultiModelSystem] Created and initialized provider: ${config.type}-${config.model}`,
    );

    return provider;
  }

  private async getOrCreateUtilityProvider(
    providerType: ModelProviderType,
    utilityModel: string,
  ): Promise<BaseModelProvider> {
    ...
    console.log(
      `[MultiModelSystem] Created utility provider: ${providerType}-${utilityModel}`,
    );
  ...
  private async setProviderTools(): Promise<void> {
    ...
    try {
      ...
      provider.setTools();
      console.log(
        `[MultiModelSystem] Tools set for provider: ${this.currentProvider.type}`,
      );
    } catch (error) {
      console.error(
        `[MultiModelSystem] Failed to set tools for provider: ${this.currentProvider.type}`,
        error,
      );
    }
  }
  ...
  private async tryCompressChat(
    ...
    console.log(
      `[MultiModelSystem] tryCompressChat called: force=${force}, history.length=${historyArray.length}`,
    );
    ...
    const conversationMessages = historyArray.filter(
      (m) => m.role !== 'system',
    );
    ...
    console.log(
      `[MultiModelSystem] Turn count check: ${userTurns} user turns vs ${TURN_COMPRESSION_THRESHOLD} threshold`,
    );
    ...
    if (!force) {
      ...
      const totalChars = historyArray.reduce(
      ...
      if (
        estimatedTokens < thresholdTokens * 0.9 &&
        userTurns < TURN_COMPRESSION_THRESHOLD
      ) {
        console.log(
          `[MultiModelSystem] Compression skipped: estimated ${estimatedTokens.toFixed(0)} tokens (${totalChars} chars) < ${(thresholdTokens * 0.9).toFixed(0)} threshold, ${userTurns} turns < ${TURN_COMPRESSION_THRESHOLD}`,
        );
        return {
      ...
      console.log(
        `[MultiModelSystem] Estimated ${estimatedTokens.toFixed(0)} tokens (${totalChars} chars), may need compression, doing precise token count...`,
      );
    }
    ...
    try {
      if (provider.countTokens) {
        ...
        console.warn(
          `[MultiModelSystem] Provider does not support token counting, skipping compression`,
        );
        return {
          originalTokenCount: 0,
          newTokenCount: 0,
    ...
    if (originalTokenCount === 0) {
      console.warn(
        `[MultiModelSystem] Could not determine token count for compression.`,
      );
      return {
        originalTokenCount: 0,
        newTokenCount: 0,
    ...
    if (!force) {
      ...
      console.log(
        `[MultiModelSystem] Compression triggered by ${compressionReason}`,
      );
    } else {
    ...
    console.log(
      `[MultiModelSystem] Starting compression. Original tokens: ${originalTokenCount}, limit: ${tokenLimit}`,
    );
    ...
    if (pendingToolCalls.length > 0) {
      console.log(
        `[MultiModelSystem] Found ${pendingToolCalls.length} incomplete tool calls at split point, adjusting...`,
      );
      ...
      for (let i = compressBeforeIndex - 1; i >= 0; i--) {
        ...
        if (
          ...
          const hasMatchingCall = msg.toolCalls.some((tc) =>
            pendingToolCalls.includes(tc.id),
          );
          if (hasMatchingCall) {
            // Move split point to before this assistant message
            compressBeforeIndex = i;
            console.log(
              `[MultiModelSystem] Moved split point to index ${i} to preserve tool call integrity`,
            );
            break;
          }
        }
    ...
    console.log(
      `[MultiModelSystem] Compressing ${historyToCompress.length} messages, keeping ${historyToKeep.length} messages`,
    );

    try {
      // Create compression request - filter system messages from history to compress
      const filteredHistoryToCompress = historyToCompress.filter(
        (msg) => msg.role !== 'system',
      );
      ...
      const response = await provider.sendCompressionMessage(
        compressionMessages,
        new AbortController().signal,
      );

      // Extract only the state_snapshot part from the response
      const stateSnapshotMatch = response.content.match(
      ...
      console.log(
        `[MultiModelSystem] Updated SessionManager with compressed history (${balancedHistory.length} messages, balanced function calls)`,
      );
      ...
      try {
        ...
        console.warn(
          '[MultiModelSystem] Could not determine compressed history token count:',
          error,
        );
      ...
      if (newTokenCount > originalTokenCount) {
        console.warn(
          `[MultiModelSystem] Compression failed - tokens increased from ${originalTokenCount} to ${newTokenCount}`,
        );
        // Revert to original history
        sessionManager.setHistory(historyArray);
        return {
      ...
      console.log(
        `[MultiModelSystem] Compression successful. Tokens: ${originalTokenCount} -> ${newTokenCount}`,
      );
      return {
        originalTokenCount,
        newTokenCount,
  ...
  private saveAssistantWithToolCalls(
    ...
    console.log(
      `[MultiModelSystem] Saving assistant message: ${allToolCalls.length} total tool calls, ${executedToolCalls.length} executed, ${executedToolResponses.length} responses`,
    );
    ...
    console.log(
      `[MultiModelSystem] Saved assistant message with ${executedToolCalls.length} tool calls and ${executedToolResponses.length} responses`,
    );
  }
   ...
   */
  private fixFunctionCallResponseBalance(
    ...
    for (const message of history) {
      if (
        ...
        if (matchingCallIndex !== -1) {
          ...
          console.warn(
            `[MultiModelSystem] Skipping orphaned tool response: ${message.tool_call_id}`,
          );
        }
      } else {
        ...
        if (pendingToolCalls.length > 0) {
          console.warn(
            `[MultiModelSystem] Removing ${pendingToolCalls.length} unmatched tool calls before non-tool message`,
          );
          pendingToolCalls = [];
        }
        balancedHistory.push(message);
    ...
    if (pendingToolCalls.length > 0) {
      console.warn(
        `[MultiModelSystem] Removing final assistant message with ${pendingToolCalls.length} unmatched tool calls`,
      );
    ...
    console.log(
      `[MultiModelSystem] Function call balance fixed: ${history.length} -> ${balancedHistory.length} messages`,
    );
  ...
  private async enhanceMessagesWithRole(
    ...
    const userMemory = systemMessages.find((m) =>
    ...
    const workspaceContext =
      await this.workspaceManager.getEnvironmentContext();
