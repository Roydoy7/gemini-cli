...
interface PDFParams {
  ...
  op: 'create' | 'merge' | 'split' | 'extracttext' | 'search' | 'info';
...
class PDFInvocation extends BaseToolInvocation<PDFParams, PDFResult> {
  /**
   * PDF operations are safe for non-interactive execution in subagents.
   * Read operations (info, search, extracttext) are read-only.
   * Write operations (create, merge, split) create new files without modifying existing ones.
   */
...
${outputFiles
  .map(
    (filePath, index) => `${index + 1}. ${path.basename(filePath)}
   Path: ${filePath}`,
  )
  .join('\n')}`,
    ...
    try {
      ...
      for (const pageNum of pageNumbers) {
        try {
          ...
          pageTexts[pageNum] =
            `[Error extracting page: ${pageError instanceof Error ? pageError.message : 'Unknown error'}]`;
        }
      }
...
${text}`,
  ...
  private async searchText(
    ...
    try {
      ...
      if (liveOutputCallback) {
        liveOutputCallback(
          `Starting search for "${query}" in ${pageNumbers.length} pages...`,
        );
      }
      ...
      if (pageNumbers.length > 20) {
        console.log(
          `[PDF Search] Using ${Math.min(4, pageNumbers.length)} worker threads for parallel processing...`,
        );
        return await this.searchWithWorkers(
          file,
          query,
          pageNumbers,
          liveOutputCallback,
      ...
      console.log(
        `[PDF Search] Processing ${pageNumbers.length} pages with controlled parallelism...`,
      );
      ...
      for (let i = 0; i < pageNumbers.length; i += BATCH_SIZE) {
        ...
        if (liveOutputCallback) {
          liveOutputCallback(
            `Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(pageNumbers.length / BATCH_SIZE)} (pages ${batch[0]}-${batch[batch.length - 1]})`,
          );
        }
        ...
        const batchPromises = batch.map(async (pageNum) => {
          try {
            ...
            // Convert single page to buffer and extract text
            const singlePageBuffer = await singlePageDoc.save();
            const singlePageData = await pdfParse(
              Buffer.from(singlePageBuffer),
            );
        ...
        if (liveOutputCallback) {
          const progress = Math.round(
            ((i + batch.length) / pageNumbers.length) * 100,
          );
          liveOutputCallback(
            `Completed ${i + batch.length}/${pageNumbers.length} pages (${progress}%)`,
          );
        }
      }
      ...
      if (liveOutputCallback) {
        liveOutputCallback(
          `Search completed! Found ${totalMatches} matches total.`,
        );
      }
      ...
      return {
        ...
        pageSearchResults,
        pageCount,
        llmContent: this.formatSearchResults(
          query,
          totalMatches,
          pageSearchResults,
          pageCount,
      ...
      return this.createErrorResult(
        `Failed to search PDF: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  private async searchWithWorkers(
    ...
    try {
      ...
      if (!existsSync(workerScript)) {
        console.warn(
          `[PDF Search] Worker script not found: ${workerScript}, falling back to async processing`,
        );
        if (liveOutputCallback) {
          liveOutputCallback(
            `Worker not available, using regular async processing for ${pageNumbers.length} pages...`,
          );
        }
      ...
      for (
        ...
        i += Math.ceil(pageNumbers.length / maxWorkers)
      ) {
        batches.push(
          pageNumbers.slice(i, i + Math.ceil(pageNumbers.length / maxWorkers)),
        );
      }

      if (liveOutputCallback) {
        liveOutputCallback(
          `Starting ${batches.length} worker threads for ${pageNumbers.length} pages...`,
        );
      }

      const workerPromises = batches.map(
        (batch, batchIndex) =>
          new Promise<Array<{ pageNum: number; matches: MatchResult[] }>>(
            (resolve, reject) => {
              if (signal?.aborted) {
              ...
              worker.on('message', (result) => {
                ...
                if (liveOutputCallback) {
                  ...
                  const overallProgress = Math.round(
                    (Math.min(totalProcessed, pageNumbers.length) /
                      pageNumbers.length) *
                      100,
                  );
                  liveOutputCallback(
                    `Worker ${batchIndex + 1}: Completed ${batch.length} pages (${overallProgress}%)`,
                  );
                }
      ...
      if (liveOutputCallback) {
        liveOutputCallback(
          `Multi-threaded search completed! Found ${totalMatches} matches total.`,
        );
      }
      ...
      return this.createErrorResult(
        `Multi-threaded search failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
    }
  }
  ...
  private formatSearchResults(
    ...
    const errorPages = Object.entries(pageSearchResults).filter(([, matches]) =>
      matches.some(
        (match) =>
          match.context.startsWith('[Worker Error:') ||
          match.context.startsWith('[Error'),
      ),
    );
    ...
    if (errorPages.length > 3) {
      const successfulPages =
        Object.keys(pageSearchResults).length - errorPages.length;
...
${Object.entries(pageSearchResults)
  .filter(
    ([, matches]) =>
      matches.length > 0 && !matches.some((m) => m.context.startsWith('[')),
  )
...
${errorPages.length > 0 ? `\n**Note:** ${errorPages.length} pages encountered processing errors and were skipped.` : ''}`;
    ...
    const methodInfo = isMultiThreaded
  ...
  Object.entries(pageSearchResults)
    ...
    .map(([pageNum, matches]) => {
      ...
      const validMatches = matches.filter(
        (match) => !match.context.startsWith('['),
      );
...
${totalMatches === 0 ? `The text "${query}" was not found in the searched pages.` : ''}`;
  ...
  private async getPDFInfo(file: string, pages?: string): Promise<PDFResult> {
    try {
      ...
      const pageSizeGroups = new Map<
        string,
        { size: string; rotation: number; pages: number[] }
      >();
    ...
    try {
      ...
      if (names instanceof PDFDict) {
        ...
        if (destsTree instanceof PDFDict) {
          ...
          let foundDest: PDFObject | null | undefined = this.searchNameTree(
            destsTree,
            destName,
          );
          if (foundDest) {
            ...
            if (
              typeof foundDest === 'object' &&
              foundDest !== null &&
              'objectNumber' in foundDest
            ) {
      ...
      console.warn(
        `[PDF] Failed to resolve named destination "${destName}":`,
        error,
      );
      return null;
    }
        ...
        try {
          ...
          while (current) {
            try {
              ...
              // Get title
              const titleObj = currentDict.lookup(
                currentDict.context.obj('Title'),
              );
              let title = 'Untitled';
              ...
              if (titleObj) {
                ...
                if (titleObj instanceof PDFString) {
                  ...
                  typeof (titleObj as { asString: unknown }).asString ===
                    'function'
                ) {
                  rawTitle = (
                    titleObj as { asString: () => string }
                  ).asString();
                } else {
              ...
              const findPageNumber = (pageRef: PDFObject): number => {
                ...
                try {
                  ...
                  const pageByStr = pages.findIndex(
                    (page) => String(page.ref) === refStr,
                  );
                  if (pageByStr !== -1) {
                    return pageByStr + 1;
                  }
              ...
              if (dest) {
                ...
                if (actionDict && actionDict instanceof PDFDict) {
                  ...
                  const actionDest = actionDict.lookup(
                    actionDict.context.obj('D'),
                  );
                  if (actionDest) {
                    ...
                    if (actionDest instanceof PDFString) {
                      ...
                      const resolvedDest = this.resolveNamedDestination(
                        pdfDoc,
                        destName,
                      );
                      if (resolvedDest) {
              ...
              const firstChild = currentDict.lookup(
                currentDict.context.obj('First'),
              );
              ...
              current =
                currentDict.lookup(currentDict.context.obj('Next')) || null;
            } catch (itemError) {
              console.warn(
                '[PDF Outline] Error parsing outline item:',
                itemError,
              );
              break;
            }
          ...
          console.warn(
            '[PDF Outline] Error parsing outline level:',
            levelError,
          );
        }
    ...
    try {
      ...
      if (existsSync(dirPath)) {
        ...
        const similarFiles = pdfFiles.filter((f: string) => {
          ...
          return (
            normalizedFile.includes(normalizedInput.substring(0, 10)) ||
...
- Manipulating PDF content (edit, annotate, sign) is not supported.
      ...
      Kind.Other,
      {
        ...
        properties: {
          ...
          op: {
            type: 'string',
            enum: ['create', 'merge', 'split', 'extracttext', 'search', 'info'],
            description:
              'Operation: extracttext=get text from specific pages (use pages param like "1-5" or "1,3,5" to avoid processing entire document), search=find text (pages param optional to limit search scope), split=save specified pages as separate PDFs, info=get PDF metadata, create=make new PDF, merge=combine PDFs',
          },
          pages: {
            type: 'string',
            description:
              'RECOMMENDED: Page numbers/ranges (1-based) like "1-5", "1,3,5", or "1-3,8-10". Saves time by avoiding full document processing. If omitted, processes ALL pages',
          },
          query: {
            type: 'string',
            description: 'Search text query (REQUIRED for search operation)',
          },
          output: {
            type: 'string',
            description:
              'Output file path (for extracttext: saves text to file; for split/create: output PDF file)',
          },
          ...
          flatten: {
            type: 'boolean',
            description: 'Flatten forms after filling',
          },
          sources: {
            type: 'array',
            items: { type: 'string' },
  ...
  protected createInvocation(
    params: PDFParams,
  ): ToolInvocation<PDFParams, PDFResult> {
