...
class FileOpsInvocation extends BaseToolInvocation<
  FileOpsParams,
  FileOpsResult
> {
  constructor(
  ...
  getDescription(): string {
    ...
    if (op === 'batchFile') {
      ...
      const sourceDesc =
        count > 0 ? `${count} files` : `files${patternText}${sourceDirText}`;
    ...
    if (op === 'batchDir') {
      ...
      const sourceDesc =
        count > 0
          ? `${count} directories`
          : `directories${patternText}${sourceDirText}`;
  ...
  private async renameOrMove(signal: AbortSignal): Promise<FileOpsResult> {
    ...
    return this.createSuccessResult(
      `Successfully ${this.params.op === 'renameFile' ? 'renamed' : 'moved'} "${path.basename(source)}" to "${path.basename(target)}"`,
    );
  }
  ...
  private async copyFile(signal: AbortSignal): Promise<FileOpsResult> {
    ...
    return this.createSuccessResult(
      `Successfully copied "${path.basename(source)}" to "${path.basename(target)}"`,
    );
  }
  ...
  private async deleteFile(signal: AbortSignal): Promise<FileOpsResult> {
    ...
    if (stat.isDirectory()) {
      throw new Error(
        `"${source}" is a directory, use rmdir operation instead`,
      );
    }
    ...
    return this.createSuccessResult(
      `Successfully deleted "${path.basename(source)}"`,
    );
  }
  ...
  private async createDirectory(signal: AbortSignal): Promise<FileOpsResult> {
    ...
    return this.createSuccessResult(
      `Successfully created directory "${path.basename(targetDir)}"`,
    );
  }
  ...
  private async removeDirectory(signal: AbortSignal): Promise<FileOpsResult> {
    ...
    if (!stat.isDirectory()) {
      throw new Error(
        `"${targetDir}" is not a directory, use delete operation instead`,
      );
    }
    ...
    return this.createSuccessResult(
      `Successfully removed directory "${path.basename(targetDir)}"`,
    );
  }
  ...
  private async batchOperation(signal: AbortSignal): Promise<FileOpsResult> {
    const { operation, sources, targetDir, pattern, target, sourceDir } =
      this.params;
    ...
    if (sources && sources.length > 0) {
      ...
      throw new Error(
        'Either sources array or pattern required for batch operation',
      );
    }
    ...
    const details = results
      .map((r) => {
        ...
        if (r.success) {
          const targetName = r.targetPath
            ? path.basename(r.targetPath)
            : 'target';
  ...
  private async batchDirOperation(signal: AbortSignal): Promise<FileOpsResult> {
    ...
    if (['move', 'copy'].includes(operation)) {
      if (!targetDir) {
        throw new Error(
          `targetDir required for batchDir ${operation} operation`,
        );
      }
    ...
    if (sources && sources.length > 0) {
      ...
      throw new Error(
        'Either sources array or pattern required for batchDir operation',
      );
    }
  ...
  private async readFile(signal: AbortSignal): Promise<FileOpsResult> {
    ...
    if (fileType === 'image') {
      ...
      // Read text file with optional pagination (auto-detect encoding)
      const fileSystemService = this.config.getFileSystemService();
      const content = await fileSystemService.readTextFile(file_path);
      ...
      if (offset !== undefined || limit !== undefined) {
        ...
        if (startLine >= totalLines) {
          throw new Error(
            `Offset ${startLine} exceeds file length (${totalLines} lines)`,
          );
        }
...
${resultContent}`;
      ...
      throw new Error(
        `Unsupported file type for reading: ${fileName}. Only text and image files are supported.`,
      );
    }
  }
  ...
  private detectFileType(filePath: string): 'text' | 'image' | 'other' {
    ...
    if (
      ...
      (lookedUpMimeType &&
        ...
        lookedUpMimeType.startsWith('text/'))
    ) {
      return 'text';
    }
  ...
  private isDestructiveOperation(params: FileOpsParams): boolean {
    ...
    if (
      (op === 'batchFile' || op === 'batchDir') &&
      ['delete', 'move'].includes(operation || '')
    ) {
      return true;
    }
    ...
    if (
      (op === 'batchFile' || op === 'batchDir') &&
      ['copy', 'rename'].includes(operation || '')
    ) {
      return true;
    }
...
export class FileTool extends BaseDeclarativeTool<
  FileOpsParams,
  FileOpsResult
> {
      ...
      Kind.Other,
      {
        ...
        properties: {
          ...
          target: {
            type: 'string',
            description:
              'Target file or directory path (required for single-file rename/move/copy/mkdir operations, NOT used for batch operations)',
          },
          operation: {
            type: 'string',
            enum: ['move', 'copy', 'rename', 'delete'],
            description:
              'Batch operation type: move (files/directories to directory), copy (files/directories to directory), rename (files/directories with pattern), delete (remove files/directories) - ONLY used with op: "batchFile" or "batchDir"',
          },
          sources: {
            type: 'array',
            items: { type: 'string' },
          ...
          targetDir: {
            type: 'string',
            description:
              'Target directory for batch move/copy operations - REQUIRED when using op: "batch" with operation: "move" or "copy"',
          },
          sourceDir: {
            type: 'string',
            description:
              'Source directory for pattern matching in batch operations (defaults to current directory)',
          },
          pattern: {
            type: 'string',
            description:
              'Glob pattern to match files (e.g., "*.pdf", "stock_val*.xls") - used with op: "batch" and sourceDir',
          },
          content: {
            type: 'string',
            description:
              'Content to write or append to file - REQUIRED for writeFile and appendFile operations. For text files: provide text content. For image files (.png, .jpg, .gif, .webp, .bmp): provide base64-encoded content (with or without data URL prefix)',
          },
          file_path: {
            type: 'string',
            description:
              'File path for write/append/read operations - REQUIRED for writeFile, appendFile, and readFile operations',
          },
          offset: {
            type: 'number',
            description:
              'For readFile (text files): 0-based line number to start reading from. Use with limit for pagination.',
          },
          limit: {
            type: 'number',
            description:
              'For readFile (text files): maximum number of lines to read. Use with offset for pagination. Default is 2000 lines.',
          },
        },
