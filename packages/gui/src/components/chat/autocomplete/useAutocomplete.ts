...
export const useAutocomplete = ({
  ...
  const [currentMatch, setCurrentMatch] = useState<AutocompleteMatch | null>(
    null,
  );
  ...
  const showAutocomplete = useCallback(
    async (match: AutocompleteMatch, textareaElement: HTMLTextAreaElement) => {
      ...
      try {
        ...
        if (items && items.length > 0) {
          ...
          const spaceBelow =
            window.innerHeight - cursorPosition.top - cursorPosition.height;
          ...
          if (spaceBelow >= dropdownHeight + margin) {
            ...
            top = Math.max(
              margin,
              window.innerHeight - dropdownHeight - margin,
            );
          }
  ...
  const checkForAutocomplete = useCallback(
    async (text: string, cursorPos: number) => {
      ...
      if (match && textareaRef.current) {
        ...
        if (
          match.provider.name === 'Workspace Directories' &&
          match.provider.forceRefresh
        ) {
  ...
  const refreshAutocomplete = useCallback(async () => {
    ...
    // Force refresh the current provider if it supports it
    if (currentMatch.provider.forceRefresh) {
      await currentMatch.provider.forceRefresh();

      // Reload items after refresh
      const items = await currentMatch.provider.getItems(currentMatch.query);
      if (items && items.length > 0) {
        setItems(items);
        setSelectedIndex(0);
      }
